...
function InstalarApp({C}){
  const [deferredPrompt,setDeferredPrompt]=useState(null);
  const [instalado,setInstalado]=useState(false);
  const [plataforma,setPlataforma]=useState('outro'); // 'android'|'ios'|'desktop'|'outro'
  const [passoIos,setPassoIos]=useState(false);

  useEffect(()=>{
    // Detectar se já está instalado como PWA
    if(window.matchMedia('(display-mode: standalone)').matches||window.navigator.standalone===true){
      setInstalado(true); return;
    }
    // Detectar plataforma
  ...
  const cardSt={background:C.bg2,border:`1px solid ${C.bord}`,borderRadius:16,padding:'1.5rem',marginTop:'1.2rem'};
  const btnSt={display:'flex',alignItems:'center',justifyContent:'center',gap:8,width:'100%',padding:'.85rem',background:`linear-gradient(135deg,${C.accent},${C.accentL}80)`,color:C.bg0,fontFamily:'inherit',fontSize:'.95rem',fontWeight:700,border:'none',borderRadius:12,cursor:'pointer',marginTop:'1rem'};
  const stepSt={display:'flex',alignItems:'flex-start',gap:10,padding:'.65rem .9rem',background:C.bg3,borderRadius:10,marginBottom:'.5rem'};
  const numSt={minWidth:22,height:22,borderRadius:'50%',background:C.accent,color:C.bg0,fontSize:'.72rem',fontWeight:800,display:'flex',alignItems:'center',justifyContent:'center',marginTop:1,flexShrink:0};
  ...
  if(instalado) return(
    <div style={cardSt}>
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <div>
          <p style={{fontSize:'.9rem',fontWeight:700,color:C.t0,marginBottom:'.18rem'}}>App instalado!</p>
          <p style={{fontSize:'.75rem',color:C.t2,lineHeight:1.5}}>O CofRe já está na sua tela de início. Acesse direto sem abrir o navegador.</p>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={cardSt}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:'1rem'}}>
        ...
        <div>
          <p style={{fontSize:'.9rem',fontWeight:700,color:C.t0,marginBottom:'.15rem'}}>Instalar como Aplicativo</p>
          <p style={{fontSize:'.74rem',color:C.t2,lineHeight:1.5}}>Acesse o CofRe direto da sua tela inicial, sem precisar abrir o navegador. Funciona offline para visualizar suas senhas.</p>
        </div>
      </div>

      {/* Benefícios */}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.5rem',marginBottom:'1rem'}}>
        {[['⚡','Acesso rápido','Ícone na tela inicial'],['🔒','Mais seguro','Sem barra de endereço'],['📵','Offline','Veja senhas salvas'],['🎨','Nativo','Experiência de app']].map(([ico,t,d])=>(
          <div key={t} style={{background:C.bg3,border:`1px solid ${C.bord}`,borderRadius:10,padding:'.65rem .8rem'}}>
            <div style={{fontSize:'1.1rem',marginBottom:'.2rem'}}>{ico}</div>
            <div style={{fontSize:'.76rem',fontWeight:700,color:C.t0,marginBottom:'.1rem'}}>{t}</div>
            <div style={{fontSize:'.67rem',color:C.t2,lineHeight:1.4}}>{d}</div>
          </div>
        ))}
      </div>

      {/* Android / Desktop: botão nativo */}
      {(plataforma==='android'||plataforma==='desktop')&&deferredPrompt&&(
        <button style={btnSt} onClick={instalar}>
          📲 Instalar o CofRe agora
        </button>
      )}

      {/* Android sem prompt ainda */}
      {plataforma==='android'&&!deferredPrompt&&!passoIos&&(
        <div style={{padding:'.75rem',background:C.bg3,borderRadius:10,marginTop:'.5rem'}}>
          <p style={{fontSize:'.78rem',color:C.t2,lineHeight:1.55}}>
            No <b style={{color:C.t0}}>Chrome</b>, toque no menu <b style={{color:C.t0}}>⋮</b> (três pontos) no canto superior direito e selecione <b style={{color:C.accent}}>"Adicionar à tela inicial"</b>.
          </p>
        </div>
      )}

      {/* iOS: passo a passo */}
      {plataforma==='ios'&&!passoIos&&(
        <button style={{...btnSt,background:C.bg3,color:C.t0,border:`1.5px solid ${C.bord}`}} onClick={()=>setPassoIos(true)}>
          Ver como instalar no iPhone / iPad
        </button>
      )}
      {plataforma==='ios'&&passoIos&&(
        <div style={{marginTop:'.8rem'}}>
          <p style={{fontSize:'.78rem',fontWeight:700,color:C.t1,marginBottom:'.6rem',textTransform:'uppercase',letterSpacing:'.04em'}}>Passo a passo — Safari</p>
          {[
            ['1','Abra esta página no','Safari','(não funciona no Chrome do iOS)'],
            ['2','Toque no ícone de compartilhar','⬆️','na barra inferior do Safari'],
            ['3','Role para baixo e toque em','Adicionar à Tela de Início',''],
            ['4','Confirme o nome','CofRe','e toque em Adicionar'],
          ].map(([n,a,b,c])=>(
            <div key={n} style={stepSt}>
              <div style={numSt}>{n}</div>
              <p style={{fontSize:'.78rem',color:C.t1,lineHeight:1.5,margin:0}}>
                {a} <b style={{color:C.accent}}>{b}</b> {c}
              </p>
            </div>
          ))}
          <p style={{fontSize:'.7rem',color:C.t3,marginTop:'.5rem',lineHeight:1.5}}>
            Após instalar, o CofRe aparecerá na sua tela inicial com ícone próprio, como um app nativo.
          </p>
        </div>
      )}

      {/* Desktop */}
      {plataforma==='desktop'&&!deferredPrompt&&(
        <div style={{padding:'.75rem',background:C.bg3,borderRadius:10,marginTop:'.5rem'}}>
          <p style={{fontSize:'.78rem',color:C.t2,lineHeight:1.55}}>
            No <b style={{color:C.t0}}>Chrome</b> ou <b style={{color:C.t0}}>Edge</b>, clique no ícone <b style={{color:C.accent}}>⊕</b> na barra de endereço ou acesse <b style={{color:C.t0}}>Menu → Instalar CofRe</b>.
          </p>
        </div>
      )}
    </div>
