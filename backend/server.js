...
const allowedOrigins = [
  ...
  'https://cofre-backend.onrender.com',
...
app.post('/api/auth/login', authLimiter, [
  ...
  try {
    ...
    await query('INSERT INTO sessoes (usuario_id,token_hash,ip_origem,expira_em) VALUES (?,?,?,?)',
      [parseInt(u.id),th,ip,new Date(Date.now()+8*3600*1000)]);
    query('INSERT INTO historico_sessoes (usuario_id,email_hash,ip_origem,user_agent,plano_ativo) VALUES (?,?,?,?,?)',
      [parseInt(u.id),eh,ip,ua,planoNome]).catch(()=>{});
...
app.get('/api/usuario/perfil', autenticar, async (req,res) => {
  const [r]=await query('SELECT email_enc,nome,sobrenome,criado_em FROM usuarios WHERE id=?',[parseInt(req.user.sub)]);
...
app.patch('/api/usuario/perfil', autenticar, [
  ...
  await query('UPDATE usuarios SET nome=COALESCE(?,nome),sobrenome=COALESCE(?,sobrenome) WHERE id=?',
    [req.body.nome??null,req.body.sobrenome??null,parseInt(req.user.sub)]);
  res.json({mensagem:'Atualizado.'});
});

app.get('/api/usuario/preferencias', autenticar, async (req,res) => {
  const [u]=await query('SELECT email_hash FROM usuarios WHERE id=? LIMIT 1',[parseInt(req.user.sub)]);
...
app.get('/api/categorias', autenticar, async (req,res) => {
  const [r]=await query(
    'SELECT id,nome_enc,icone,eh_sistema,usuario_id FROM categorias_credenciais WHERE eh_sistema=true OR usuario_id=? ORDER BY eh_sistema DESC,criado_em ASC',[parseInt(req.user.sub)]);
  res.json(r);
});
app.post('/api/categorias', autenticar, [body('nomeEnc').notEmpty(),body('icone').notEmpty()], validar, async (req,res) => {
  const [r]=await query('INSERT INTO categorias_credenciais (usuario_id,nome_enc,icone,eh_sistema) VALUES (?,?,?,false) RETURNING id',
    [parseInt(req.user.sub),req.body.nomeEnc,req.body.icone]);
...
app.delete('/api/categorias/:id', autenticar, async (req,res) => {
  const [c]=await query('SELECT id FROM categorias_credenciais WHERE id=? AND usuario_id=? AND eh_sistema=false',
    [parseInt(req.params.id),parseInt(req.user.sub)]);
  if(!c.length) return res.status(404).json({erro:'Não encontrada.'});
  await query('DELETE FROM categorias_credenciais WHERE id=?',[parseInt(req.params.id)]);
...
app.get('/api/cofre', autenticar, async (req,res) => {
  const [r]=await query(
    `SELECT c.id,c.categoria_id,cc.nome_enc AS categoria_nome_enc,cc.icone,cc.eh_sistema,
     ...
     WHERE c.usuario_id=? AND c.excluido_em IS NULL ORDER BY c.favorito DESC,c.criado_em DESC`,[parseInt(req.user.sub)]);
  res.json(r);
});
app.post('/api/cofre', autenticar, [body('categoriaId').isInt({min:1}),body('tituloEnc').notEmpty(),body('senhaEnc').notEmpty()], validar, async (req,res) => {
  ...
  const [r]=await query(
    `INSERT INTO credenciais (usuario_id,categoria_id,titulo_enc,login_enc,senha_enc,url_ou_banco_enc,
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) RETURNING id`,
    [parseInt(req.user.sub),categoriaId,tituloEnc,loginEnc??null,senhaEnc,urlOuBancoEnc??null,
...
app.put('/api/cofre/:id', autenticar, async (req,res) => {
  const [c]=await query('SELECT id FROM credenciais WHERE id=? AND usuario_id=? AND excluido_em IS NULL',[parseInt(req.params.id),parseInt(req.user.sub)]);
  if(!c.length) return res.status(404).json({erro:'Não encontrada.'});
  ...
  await query(
    ...
    [tituloEnc,loginEnc??null,senhaEnc,urlOuBancoEnc??null,senha4digEnc??null,senha6digEnc??null,
     ...
     camposExtrasEnc??null,categoriaId,!!favorito,parseInt(req.params.id)]);
  res.json({mensagem:'Atualizado.'});
});
app.delete('/api/cofre/:id', autenticar, async (req,res) => {
  const [c]=await query('SELECT id FROM credenciais WHERE id=? AND usuario_id=?',[parseInt(req.params.id),parseInt(req.user.sub)]);
  if(!c.length) return res.status(404).json({erro:'Não encontrada.'});
  await query('UPDATE credenciais SET excluido_em=NOW() WHERE id=?',[parseInt(req.params.id)]);
